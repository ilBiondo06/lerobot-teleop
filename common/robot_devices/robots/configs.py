...
"""from lerobot.common.robot_devices.controller.movelladot import (
    MovellaDotConfig
)"""
...
@RobotConfig.register_subclass("so100MovellaDot")
@dataclass
class So100RobotMovellaDotConfig(ManipulatorRobotConfig):
    calibration_dir: str = ".cache/calibration/so100"
    # `max_relative_target` limits the magnitude of the relative positional target vector for safety purposes.
    # Set this to a positive scalar to have the same value for all motors, or a list that is the same length as
    # the number of motors in your follower arms.
    ...
    leader_arms: dict[str, MotorsBusConfig] = field(
        default_factory=lambda: {
            "main": MovellaDotMotorsBusConfig(
                port="/tmp/ttyV0", # TODO: INSERT YOUR VIRTUAL PORT USING A BRIDGE.
                motors={
                    # name: (index, model)
                    "shoulder_pan": [1, "sts3215"],
                    "shoulder_lift": [2, "sts3215"],
