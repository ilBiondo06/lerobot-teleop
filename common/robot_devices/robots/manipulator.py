...
class ManipulatorRobot:
    ...
    def connect(self):
        ...
        elif self.robot_type in ["so100MovellaDot"]:
            from lerobot.common.robot_devices.motors.movelladot import TorqueMode

        print("check",self.robot_type in ["so100MovellaDot"])
        print("DEDUG ROBOT TYPE", self.robot_type)
        # We assume that at connection time, arms are in a rest position, and torque can
        # be safely disabled to run calibration and/or set robot preset configurations.
        
        if self.robot_type in ["so100MovellaDot"]:
            print("DEDUG 1 ROBOT TYPE", self.robot_type)
            for name in self.follower_arms:
                self.follower_arms[name].write("Torque_Enable", TorqueMode.DISABLED.value)
        else:
            print("DEDUG 2 ROBOT TYPE", self.robot_type)
            for name in self.follower_arms:
                self.follower_arms[name].write("Torque_Enable", TorqueMode.DISABLED.value)
            for name in self.leader_arms:
        ...
        elif self.robot_type in ["so100", "so101", "moss", "lekiwi","so100MovellaDot"]:
            self.set_so100_robot_preset()
        # For movelladot there is not a preset
    ...
    def activate_calibration(self):
        ...
        def load_or_run_calibration_(name, arm, arm_type):
            ...
            if arm_calib_path.exists():
                ...
                elif self.robot_type in ["so100", "so101", "moss", "lekiwi", "so100MovellaDot"]: # ADDED "so100MovellaDot"
    ...
    def teleop_step(
        ...
        for name in self.leader_arms:
            ...
            if False: # HARDCODE
                ...
                leader_pos[name] = self.leader_arms[name].read()
            # TODO: ricordarsi che questa parte va bene solo per movella, e quindi devi rimettere la parte precedente se vuoi teleoperare con qualcos'altro
            # leader_pos[name] = torch.from_numpy(np.array(leader_pos[name]))
            #arr = np.array(leader_pos[name], dtype=np.float32)
            print("leader_pos[name]",leader_pos[name])
            
            if isinstance(leader_pos[name], dict):
                arr = np.array(list(leader_pos[name].values()), dtype=np.float32)
            elif isinstance(leader_pos[name], list):
                arr = np.array(leader_pos[name], dtype=np.float32)
            print("arr",arr)
            leader_pos[name] = torch.from_numpy(arr)
